...
public class AssetBundleMgr : MonoSingleton<AssetBundleMgr> {
   ...
   // private string m_BaseDownLoadingURL = "";
    ...
    public void Initialize(Action initOK = null) {
        SetManifestName();
        if (Application.platform == RuntimePlatform.WindowsEditor && !m_isAbLoad) {
            if (initOK != null) {
                initOK();
            }
            return;
    ...
    public void LoadAsset<T>(string abName, string assetName, Action<UObject> action = null) where T : UObject {
        if (Application.platform == RuntimePlatform.WindowsEditor && !m_isAbLoad) {
            LoadFormAB<T>(abName, assetName, action);
        }
    }
    ...
    private IEnumerator DownLoad(string abName, Type type = null) {
        ...
        if (type == typeof(AssetBundleManifest)) {
            ...
            www = WWW.LoadFromCacheOrDownload(url, m_abManifest.GetAssetBundleHash(abName), 0);
        }
        yield return www;
        if (www.error != null) {
    ...
    public void AddAssetBundleReference(string abName) {
        if (!string.IsNullOrEmpty(abName)) {
            abName = GetRealAssetPath(name);
            if (m_loadAssetDic.ContainsKey(abName)) {
                m_loadAssetDic[abName].m_referenceCount++;
                AddDependenciesReference(abName);
            }
        }
    }
    ...
    private string GetRealAssetPath(string abName) {
        ...
        if (Application.platform == RuntimePlatform.WindowsEditor && !m_isAbLoad) {
            return abName.Substring(0, abName.LastIndexOf(@"/") + 1);
        } else {
